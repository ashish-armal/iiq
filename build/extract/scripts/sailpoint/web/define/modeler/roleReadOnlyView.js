...
var roleCategoryTemplate = new Ext.XTemplate(
    '<div>',
      '<table class="paddedTbl width100">',
        '<tpl for="attributes">',
          '<tr>',
            '<td style="border: 1px solid #CCC"><span class="titleColumn">','{displayName:htmlEncode}','<tpl if="missing">','*','</tpl>','</span></td>',
            '<td style="border: 1px solid #CCC"><span>','<tpl if="value != null">','{value:htmlEncode}','</tpl>','</span></td>',
          '</tr>',
        '</tpl>',
      '</table>',
    '</div>'
...
SailPoint.Role.RoleView.populate = function(viewConfig) {
    ...
    viewDiv.setVisibilityMode(Ext.Element.DISPLAY);
    roleDataDiv.setVisibilityMode(Ext.Element.DISPLAY);
    directEntitlementsDiv.setVisibilityMode(Ext.Element.DISPLAY);
    ...
    if (results.roleName) {
        ...
        for (var categoryName in results.roleExtendableAttributesByCategory) {
            var child = Ext.create('Ext.panel.Panel', {
                ...
                anchor:'98%',
                border:false,
                title: SailPoint.Utils.escapeForXss(categoryName),
                collapsible:true,
                collapsed:false,
                autoScroll:true,
                style: 'padding: 10px;',
        ...
        if (hasEntitlements) {
            ...
            if (results.hasSimpleEntitlements) {
                ...
                if(prefix === 'archive'){
                    simpleEntitlementsGridDataStore.getProxy().extraParams = {archiveId: storeParams.archiveId, prefix: "archive" };
                }
                //IIQETN-5561 :- When clicking in a new node we need to refresh the current page for simple entitlements
                simpleEntitlementsGridDataStore.currentPage = 1;
        ...
        SailPoint.Role.RoleView.displayIIQRights(viewConfig, typeDefinition);
...
SailPoint.Role.RoleView.displayIIQRights = function(viewConfig, typeDefinition) {
    var prefix = viewConfig.prefix,
        results = viewConfig.results,
        roleRightsSection = Ext.getCmp(prefix + 'sailpointRightsSectionPanel'),
        rightsDiv = Ext.get(prefix + 'sailpointRights'),
        warningOptions = {
            contentDiv: prefix + 'sailpointRightsInvalidValuesWarning',
        };
    
    // reset warning message by hiding
    SailPoint.Role.RoleView.hideInvalidFieldWarning(warningOptions);
    // IIQCB-1404 render IIQ rights no matter what typeDefinition states
    if ( results.capabilities != null || results.scopes != null ) {
        roleRightsTemplate.overwrite(rightsDiv, results);
        roleRightsSection.show();
        
        // if typeDefinition doesn't allow IIQ rights, show warning
        if ( typeDefinition && typeDefinition.noIIQ )  {
            SailPoint.Role.RoleView.addFieldValuesWarning(results.capabilities, warningOptions);
            SailPoint.Role.RoleView.addFieldValuesWarning(results.scopes, warningOptions);
            SailPoint.Role.RoleView.displayInvalidFieldValuesWarning(warningOptions);
        }
    } else {
...
SailPoint.Role.RoleView.addFieldValuesWarning = function(arrayNameObjects, warningOptions) {
    if (!warningOptions.values) {
        warningOptions.values = [];
    }
    Ext.Array.each(arrayNameObjects, function(item, index, allItems) {
        Ext.Array.push(warningOptions.values, item.name);
    });
}
...
SailPoint.Role.RoleView.displayRoleReference = function(displayOptions) {
    ...
    if (isShown) {
        if (isInvalid) {
            ...
            SailPoint.Role.RoleView.hideInvalidFieldWarning(warningOptions);
            Ext.get(warningDiv).setVisibilityMode(Ext.Element.DISPLAY).hide();
        ...
        if (invalidValuesWarningOptions.values.length > 0) {
            ...
            Ext.get(warningValuesDiv).setVisibilityMode(Ext.Element.DISPLAY).hide();
...
SailPoint.Role.RoleView.getRoleViewContentPanel = function(config) {
    ...
    var includedEntitlementsSectionSimpleConfig = SailPoint.Role.RoleView.createSimpleEntitlementPanel({
        ...
        columns: [
            { dataIndex: 'applicationName', header: '#{msgs.role_simple_entitlement_application_name_header}', flex: 1, sortable: true },
            { dataIndex: 'roleName', header: '#{msgs.role_simple_entitlement_role_header}', flex: 1, sortable: true, renderer: 'htmlEncode' },
            { dataIndex: 'property', header: '#{msgs.role_simple_entitlement_property_header}', flex: 1, sortable: true },
            { dataIndex: 'displayValue', header: '#{msgs.role_simple_entitlement_value_header}', flex: 3, sortable: true, renderer: 'htmlEncode' }
        ],
        disableSelect: true
    ...
    var rightsSectionConfig = {
        ...
        items: [{
            xtype: 'container',
            id: prefix + 'sailpointRightsInvalidValuesWarningContainer',
            contentEl: prefix + 'sailpointRightsInvalidValuesWarning'
        }, {
            xtype: 'container',
            id: prefix + 'sailpointRightsContainer',
    ...
    if (isMainView) {
        contentPanelConfig.bbar = [
            new Ext.Button({
                ...
                handler: function() {
                    var viewerState = Ext.getCmp('roleViewerStateController').getState();
                    Ext.getDom('viewerForm:roleToEdit').value = viewerState.selectedRoleId || viewerState.filteredNode;
                    Ext.getDom('viewerForm:editRole').click();
                }
            })
        ];
    }
...
SailPoint.Role.RoleView.createRoleReferencePanel = function(config) {
    ...
    roleReferenceGridDataStore.on('beforeload', function(store, operation, eOpts) {
    	...
    	if (SailPoint.Role.Viewer) {
    	    ...
    	    params = { roleId: Ext.getDom('rollbackForm:roleToEdit').value };
    	}
    ...
    var roleReferenceGridPanelConfig = {
        ...
        columns: [
            { dataIndex: 'name', header: '#{msgs.name}', flex: 2, sortable: true, renderer: 'htmlEncode' },
...
//                    '<span> debug: application is {application} and attribute is {parent.name} and value is {value}</span>',
                    '<span class="unboldFakeLink" onclick="viewAccountGroup(\'{application}\', \'{parent.name}\', \'{value:htmlEncode}\');">',
                  '</tpl>',
                  '<tpl if="!isGroup">',
                    '<span>',
                  '</tpl>',
            ...
            '<div class="entitlementDescriptions" style="display:none">',
              '<tpl for="values">',
                '<span id="description_{valueId}" class="entitlementTxt">{description:htmlEncode}</span>',
              '</tpl>',
            '</div>',
          '</td></tr>',
        '</tpl>',
