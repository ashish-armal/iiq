...
SailPoint.Role.Metrics.displayIdentitiesDrilldown = function(roleId, metric) {
    ...
    var gridMetaData = Ext.JSON.decode(Ext.getDom('roleMetricsUIConfig').innerHTML);
    ...
    if (!identitiesDrilldown) {
        identitiesDrilldown = new SailPoint.grid.PagingCheckboxGrid({
            ...
            tbar: [{
                ...
                handler: function() {
                    ...
                    var isValid = true;
                    Ext.getDom('metricsDrilldownCertificationForm:excludeSelected').value = isExcludeSelected;
                    if (isExcludeSelected) {
                        Ext.getDom('metricsDrilldownCertificationForm:selectedIds').value = identitiesDrilldown.getExcludedIds().join();
                    } else {
                        Ext.getDom('metricsDrilldownCertificationForm:selectedIds').value = identitiesDrilldown.getSelectedIds().join();
                        if (Ext.getDom('metricsDrilldownCertificationForm:selectedIds').value.length === 0) {
                            Ext.MessageBox.show({
                    ...
                    if (isValid) {
                        Ext.getDom('metricsDrilldownCertificationForm:roleId').value = drilldownStore.getProxy().extraParams.roleId;
                        Ext.getDom('metricsDrilldownCertificationForm:metric').value = drilldownStore.getProxy().extraParams.metric;
                        Ext.getDom('metricsDrilldownCertificationForm:scheduleCertificationBtn').click();
                    }
                }
            }],
            pageSize: 25
        ...
        identitiesDrilldown.setTitle(Ext.String.htmlEncode(SailPoint.Role.Metrics.MetricDisplayNames[metric]));
    }
            ...
            if (!drilldownWindow) {
                drilldownWindow = new Ext.Window({
                    ...
                    buttons: [{
                        ...
                        cls : 'secondaryBtn',
                        handler: function() {
                            Ext.getCmp('identitiesDrilldownWindow').hide();
                        }
                    }]
                });
            }
...
SailPoint.Role.Metrics.displayEntitlementsDrilldown = function(roleId, metric) {
    ...
    if (!entitlementsDrilldown) {
        ...
        entitlementsDrilldown.setTitle(Ext.String.htmlEncode(SailPoint.Role.Metrics.MetricDisplayNames[metric]));
        entitlementsDrilldown.show();
    }
    ...
    Ext.Ajax.request({
        ...
        callback: function(options, success, response) {
            ...
            addDescriptionTooltipsFn(Ext.getDom('roleMetricsEntitlementsDisplay'));
