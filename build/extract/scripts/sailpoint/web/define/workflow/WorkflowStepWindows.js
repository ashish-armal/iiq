...
Ext.define('SailPoint.WorkflowStepGatewayWindow', {
  ...
  initGateway : function (windowOpen) {
    this.setTitle("#{msgs.transitions_edit_from} '" + Ext.String.htmlEncode(this.step.name) + "'");
    this.clear();
  ...
  addTransition : function(transition, index) {
    this.add({xtype:'box', id:'gw_title_'+index, autoEl: {tag:'div', html: '#{msgs.transition_to}: ' + Ext.String.htmlEncode(transition.to)}, cls:'gw_title'});
...
Ext.define('SailPoint.WorkflowStepConditionWindow', {
    ...
    initCondition : function() {
      this.setTitle("#{msgs.condition_edit_title} '" + Ext.String.htmlEncode(this.step.name) + "'");
...
Ext.define('SailPoint.WorkflowStepIconWindow', {
  ...
  initStep : function () {
    if(this.stepObj && this.stepObj.name) {
      title = Ext.String.htmlEncode(this.stepObj.name);
    }
    else  
      title = Ext.String.htmlEncode(this.stepId);
...
Ext.define('SailPoint.WorkflowStepWindow', {
    ...
    initComponent : function() {
      ...
      this.replicatorPanel = Ext.create('SailPoint.WorkflowStepReplicatorPanel', {
        id: 'replicatorPanel',
      ...
      //Only show the Replicator Panel for Subprocess action
      Ext.getCmp(this.actionRadio.id+'_subprocess').on('change', function() {
        if (this.getValue()) {
          Ext.getCmp('replicatorPanel').show();
        } else {
          Ext.getCmp('replicatorPanel').hide();
          Ext.getCmp('replicatorPanel').reset();
        }
    });
      ...
      this.detailsPanel = new Ext.FormPanel({
        ...
        items: [
          this.stepName,
          {xtype:'textarea',fieldLabel:'#{msgs.label_description}', name: 'description',itemId: 'stepDescription', labelWidth: 135, value: this.stepObj.description,width:550},
          {xtype: 'textfield',  helpText: '#{help.help_workflow_step_result_variable}', fieldLabel: '#{msgs.workflow_result_variable}', labelWidth: 135, name: 'stepResultVariable', itemId:'stepResultVariable', value: this.stepObj.resultVariable},
          {xtype: 'checkbox', helpText: '#{help.help_workflow_step_monitor}', fieldLabel: '#{msgs.process_instrumentation_monitor_step}', labelWidth: 135, name: 'monitorStep', itemId:'monitorStep', checked: this.stepObj.isMonitored},
          this.approvalArea,
          this.actionRadio,
          this.replicatorPanel
        ]
      });
    ...
    validate : function() {
      ...
      if(!this.stepName.validate())
      {
        ...
        this.tabPanel.setActiveTab('stepDetails');
      ...
      if(!this.argPanel.validate()) {
        ...
        this.tabPanel.setActiveTab('StepArgumentsPanel');
        return false;
      }

      if(!this.replicatorPanel.validate()) {
          statusBar.setText(this.errorText);
          statusBar.setStatus({
              text: this.errorText,
              iconCls: this.errorIconCls,
    ...
    loadReplicatorPanel : function() {
        this.replicatorPanel.reset();
        this.replicatorPanel.load();
        if (this.actionRadio.getMethod() !== 'subprocess') {
            this.replicatorPanel.hide();
        }
    },
    ...
    updateInvalidSteps: function() {
      ...
      if(invalidSteps.length>0) {
        invalidSteps.forEach(function (step) {
...
Ext.define('SailPoint.WorkflowArgAdvancedPanel', {
    ...
    getArgsJSON : function() {
        ...
        this.items.each(function(item) {
            var argId, argName, argRadio, argValueMethod, argValueSource, argNegateValue, argument;
            ...
            if(item.isXType('container') && item.getId().substring(0,12) == "argContainer") {
                ...
                if(argRadio) {
                  ...
                  argNegateValue = argRadio.getNegate();
                } else {
                  argValueMethod = '';
                  argValueSource = '';
                  argNegateValue = false;
                }
                ...
                if (!Ext.isEmpty(argValueSource)) {
                    ...
                    argument.negate = argNegateValue;
    ...
    save : function() {      
        this.items.each(function(item) {
            var argId, argName, argRadio, argValueMethod, argValueSource, argValueNegate, argument;
            //ensure that this is a container
            if(item.isXType('container') && item.getId().substring(0,12) == "argContainer") {
                argId = item.getComponent('argId').getValue();
                argName = item.getComponent('argName').getValue();
                ...
                if(argRadio) {
                  argValueMethod = argRadio.getMethod();
                  argValueSource = argRadio.getSource();
                  argValueNegate = argRadio.getNegate();
                } else {
                  argValueMethod = '';
                  argValueSource = '';
                  argValueNegate = false;
                }
              ...
              if(argument) {
                ...
                argument.valueMethod = argValueMethod;
                argument.valueSource = argValueSource;
                argument.negate = argValueNegate;
              } else {
                argument = new SailPoint.WorkflowArgument();
                argument.id = argId;
                argument.name = argName;
    ...
    addArg : function (arg) {
      ...
      argRadio.on('afterrender', function(radio) {
        radio.load(radio.method, radio.source, radio.negate);
    ...
    fixScriptRadio : function(radio) {
      ...
      var reference_radio = Ext.getDom(element_name + "_source_reference");
      if(reference_radio) {
        reference_radio.firstChild.style.width = '';
      } 
      var callable_radio = Ext.getDom(element_name + "_source_call");
      if(callable_radio) {
