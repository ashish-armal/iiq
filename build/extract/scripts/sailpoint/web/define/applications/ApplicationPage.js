...
Page.on('beforeSave', function() {
    ...
    if(Ext.get('appDescriptionHTML')) {
        Ext.getDom('editForm:appDescriptionsJSON').value = Ext.getCmp('appDescriptionHTMLCmp').getCleanValue();
    }
...
ApplicationPage.initCompositeDataStore = function() {
    var compositeDefinition = Ext.decode(Ext.getDom('editForm:compositeDefinition').value);
...
ApplicationPage.persistCompositeDefinition = function(){
    ...
    var destFormField = Ext.getDom('editForm:compositeDefinition');
...
ApplicationPage.showCreateCompositeAttrDialog = function showCreateCompositeAttrDialog(sourceButtonId){
    ...
    saveButton.on('click', function(){
        var appName =  this.items.get(0).getValue();
        var srcAttr = this.items.get(1).getValue();
        Ext.getDom('editForm:sourceApplication').value = appName;
        Ext.getDom('editForm:sourceAttribute').value = srcAttr;

        Ext.getDom(this.submitButtonId).click();
...
ApplicationPage.initAppDescription = function(isLangSelectEnabled, locale, readOnly) {
    if(Ext.get('appDescriptionHTML') && Ext.get("editForm:appDescriptionsJSON")) {
        Ext.create('SailPoint.MultiLanguageHtmlEditor', {
            renderTo: 'appDescriptionHTML',
            width:500,
            height:200,
            languageJSON : Ext.getDom("editForm:appDescriptionsJSON").value,
        ...
        Ext.each(btns, function(elt) { elt.setAttribute("unselectable", "on"); });
    }
};

ApplicationPage.initRevoker = function(jsonArray, isDisabled) {
    if (Ext.get('remediationSuggest')) {
        Ext.create('SailPoint.IdentitySuggest', {
...
ApplicationPage.initScope = function(rawData, isShow, isDisabled) {
    if (isShow) {
        ...
        var scopeValue = Ext.getDom('assignedScope').value;
        if ('' !== scopeValue) {
    ...
    var schemaWindow = Ext.create('Ext.window.Window', {
        ...
        buttons: [{
            ...
            handler: function() {
                ...
                //Click Button
                Ext.getDom('editForm:addSchemaBtn').click();
...
ApplicationPage.hideDiv = function(divId) {
    var operationRuleField = Ext.getDom(divId);
...
ApplicationPage.setRadioInput = function(inputID, value) {
    var i, selectedInput,
        radioInputs = document.getElementsByName(inputID);
...
ApplicationPage.scrollToAnchor = function(panelDiv) {
    var newSchemaType = Ext.getDom('editForm:hiddenSchemaObjectType').value,
        link = Ext.get(panelDiv + newSchemaType);
    if (link) {
        link.scrollTo();
    }
};
...
function saveReconfAppConfirmation(btn) {
    ...
    function funcButton (button) {
        if (button === 'yes') {
            ...
            Ext.getDom('editForm:hiddenSaveAppReconfBtn').click();
        }
    }
    Ext.MessageBox.show({
        title:'',
...
function showLoadingDiv() {
    var div = Ext.get('loadingSchemaDiv');
    if (div != null) {
        div.show();
    }
}
...
function hideLoadingDiv() {
    var div = Ext.get('loadingSchemaDiv');
    if (div != null) {
        div.setVisibilityMode(Ext.Element.DISPLAY).hide();
    }
}

function changeType(sel) {
    var connSelect = Ext.getDom('editForm:connector');
    Ext.getDom('editForm:type').value = connSelect.options[connSelect.selectedIndex].value;

    /** Persist the description **/
    if(Ext.getDom('appDescriptionHTML')) {
        Ext.getDom('editForm:appDescriptionsJSON').value = Ext.getCmp('appDescriptionHTMLCmp').getCleanValue();
    }

    Ext.getDom('editForm:changeTypeButton').click();
}
...
function enableApplicationReconfig( appTypes ){
    ...
    var dialogBox, panel;
    var oldtype = Ext.getDom('editForm:type').value;
                        ...
                        function funcButton (btn) {
                            if (btn === 'yes') {
                                ...
                                var tempComboVal = document.createElement('option'),
                                    appTypeElement = Ext.getDom('editForm:appType');
                                tempComboVal.text  = selValue;
                                tempComboVal.value = selValue;
                                appTypeElement.add(tempComboVal, null);
                                appTypeElement.value = selValue;
                                Ext.getDom('editForm:type').value = appTypeElement.options[appTypeElement.selectedIndex].value;
                                Ext.getDom('editForm:appReconfigMode').value = 'true';
                                Ext.getDom('editForm:changeTypeButton').click();
                                dialogBox.close();
                            }else{
                                dialogBox.close();
    ...
    function funcButton (btn) {
        if (btn === 'yes') {
            ...
            if ( selection && selection.length > 0){
                Ext.getDom('editForm:templateChoice').value = selection;
                Ext.getDom('editForm:updateProvisionPolicyBtn').click();
            }
        }
    }
...
function startTest(button) {
    Ext.getDom('testResultsDiv').className = 'workingText';
    Ext.getDom('testResultsDiv').innerHTML = '#{msgs.conn_test_connecting_to_app}';
    button.disabled = true;
}

function endTest() {
    Ext.getDom('editForm:testButton').disabled = false;
}

function showExtTab(tab) {
    var tabId = tab.getItemId();
    Ext.getDom('editForm:currentTab').value = tabId;
    refreshContentDisplay(tabId);

    // extra initialization by tab id
    if (tabId == 'schemaContent') {
        initSchemaTab();
        ...
        Ext.getDom('editForm:updatePasswordPanelBtn').click();
    } else if (tabId == 'accountsGridContent') {
        SailPoint.Define.Applications.AccountsGrid.adjustWidth();
        Ext.get('accountsGridContent').setVisibilityMode(Ext.Element.DISPLAY).hide();
...
function showTab(tabId) {
    ...
    if (tabId == 'schemaContent') {
        ...
        Ext.getDom('editForm:templatePanelLoadBtn').click();
...
function initSchemaTab() {
    var loaded = Ext.getDom('tabUpdateForm:schemaLoaded').value;

    if (( loaded == null ) || ( loaded.length == 0 ) || ( loaded == "false" )) {
        showLoadingDiv();
        Ext.getDom('tabUpdateForm:schemaLoaded').value = 'true';
        Ext.getDom('tabUpdateForm:tabUpdateBtn').click();
    }
}
...
function initApplicationDependency(jsonData, exclusionIds) {
    if (Ext.get('applicationDependency')) {
      ...
      dependencySuggest = new SailPoint.MultiSuggest({
          ...
          jsonData : Ext.decode(Ext.getDom('applicationDependencyData').innerHTML),
          inputFieldName : 'editForm:applicationDependencyHidden',
          exclusionIds : Ext.getDom('applicationDependencyExclusionIds').innerHTML,
          extraParams : {
              'showSync' : true
          }
      });
...
function initTabs() {
    ...
    var isRendered = Ext.get('submenu-tabs') === null;

    if (!isRendered) {

        // check if current tab is rendered
        var curTabElem = currentTab ? Ext.getDom(currentTab) : null;
        if (curTabElem) {
            ...
            // remove tab_ prefix
            showTab(tabContenEl);
            Ext.getDom('editForm:currentTab').value = tabContenEl;
        }
    }
}
...
function configureExtTabs(connectorType) {
    ...
    var detailsPanel = Ext.create('Ext.panel.Panel', {
        ...
        title: (Ext.getDom('editForm:appReconfigMode').value === 'true' ) ? '#{msgs.details}' +'<span style="color:red">' +'*'+'</span>' : '#{msgs.details}',
    ...
    if(! Ext.isEmpty(connectorType)) {
        ...
        var attributesPanel = Ext.create('Ext.panel.Panel', {
            ...
            title: (Ext.getDom('editForm:appReconfigMode').value === 'true' ) ? '#{msgs.app_tab_conf_settings}' + '<span style="color:red">' + '*' + '</span>' : '#{msgs.app_tab_conf_settings}',
            listeners: {activate: showExtTab},
            width: 768,
            html: ''});
        ...
        var schemaPanel = Ext.create('Ext.panel.Panel', {
            ...
            title: (Ext.getDom('editForm:appReconfigMode').value === 'true') ? getSchemaTitle() + '<span style="color:red">' + '*' + '</span>' : '#{msgs.schema}',
            listeners: {activate: showExtTab},
            width: 768,
        ...
        var provisioningPolicyPanel = Ext.create('Ext.panel.Panel', {
          ...
          title: (Ext.getDom('editForm:appReconfigMode').value === 'true') ? '#{msgs.templates}' + '<span style="color:red">' + '*' + '</span>' : '#{msgs.templates}',
          listeners: {activate: showExtTab},
          // remove some scroll bar ghosting when expand/collapsing
          autoScroll: false,
          html: ''});
        ...
        var editedAppId = Ext.getDom('editForm:id').value;
        ...
        if (Page.isCompositeApp) {
            appObjectTabPanel.on('tabchange', ApplicationPage.persistCompositeDefinition);
        }
...
function getSchemaTitle() {
    if ( ( Ext.getDom('editForm:appReconfigMode').value === true) && Ext.getDom('editForm:isSchemaMappingSaved').value === true ) {
        return '#{msgs.schema}';
    } else {
...
function startWizard(editMode) {
    ...
    if ( editMode ) {
        activePage = 3;
        Ext.getDom('editMode').value = "true";
        launchWizard(activePage);
    } else {
        Ext.getDom('editMode').value = "false";
        var configItem =  Ext.get('editForm:currentConfigSelection');
        if ( configItem ) {
            configItem.dom.value = "";
            Ext.getDom('editForm:switchConfig').click();
...
function launchWizard(activePage){
    ...
    var identityName, identitySuggest, configName, wizard;
    if (!activePage) {
        activePage = 0;
    }

    if (activePage == 0) {
        identityName =  Ext.get('editForm:selectedIdentity');
        if ( identityName ) {
            identityName.dom.value = "";
        }
        identitySuggest = Ext.getCmp('accountCorrelationIdentity');
        if (identitySuggest && identitySuggest.forcefullyClearValue)
            identitySuggest.forcefullyClearValue();
        configName =  Ext.get('editForm:configurationName');
        if ( configName ) {
            configName.dom.value = "";
        }

        Ext.getDom('editForm:newFilterValue').value = '';
        Ext.get('editForm:directApplicationAttributeSelection').selectedIndex = 0;
    }

    wizard = Ext.getCmp('correlationWizard');
    if (wizard) {
        Ext.getCmp('wizardPanels').getLayout().setActiveItem(activePage);
    } else {
        wizard = Ext.create('Ext.window.Window', {
            ...
            items    : [{
                ...
                bbar: ['->', {
                    ...
                    handler: function(btn, e) {
                        Ext.getDom('editForm:saveConfigButton').click();
                        btn.up('#correlationWizard').close();
                    },
                    cls: 'primaryBtn',
...
function getNextPanel(currentPanel, incr) {
    ...
    var editMode = false;
    var editMode = Ext.get('editMode');
    if ( editMode) {
        if ( editMode.dom.value == "true" ) {
            editMode = true;
        } else {
            editMode = false;
        }
    ...
    if ( incr == 1 ) {
        ...
        switch(current) {
            case 1:
                ...
                var configName =  Ext.getDom('editForm:configurationName');
                if ( configName ) {
                    if ( Ext.isEmpty(configName.value) ) {
                        var errorDiv = Ext.get('errorDiv');
                        if ( errorDiv ) {
                            errorDiv.dom.innerHTML = '#{msgs.corwiz_err_must_enter_name}';
                            errorDiv.show();
                            next = 1;
                        }
                    } else {
                        ...
                        for(var i=0;i<existingCorrelationConfigs.length;i++){
                            ...
                            if (conf.name == configName.value){
                                var errorDiv = Ext.get('errorDiv');
                                errorDiv.dom.innerHTML = '#{msgs.corwiz_duplicate_name}';
                                errorDiv.show();
                                next = 1;
                                break;
                            }
            ...
            case 5:
                ...
                Ext.getDom('editForm:addDirectButton').click();
...
function enterKeyCheck(node) {
    var submitOnEnter = new SailPoint.SubmitOnEnter(null, function(event) {
        ...
        if (Ext.EventObject.RETURN === event.keyCode) {
            ...
            var button = Ext.getDom('editForm:saveAppBtn');
            button.click();
...
function initializeNativeChangeFields(enabled) {
    if ( enabled ) {
        if ( enabled == "true" ) {
            var div = Ext.get("nativeChangeOptions");
            if ( div ) {
                div.show();
            }
        }
...
function changeNativeDetectionEnablement() {
    var checkbox = Ext.get("editForm:nativeChangeEnableCheckbox");
    if ( checkbox ) {
        var div = Ext.get("nativeChangeOptions");
        if ( div ) {
            if ( div.dom.style.display == "none" ) {
                div.show();
            } else {
                div.setVisibilityMode(Ext.Element.DISPLAY).hide();
            }
...
function initializeMaintenanceWindows(enabled) {
    if ( enabled ) {
        if ( enabled == "true" ) {
            var div = Ext.get("maintenanceWindowOptions");
            if ( div ) {
                div.show();
            }
        }
    }
    changeMaintenanceWindowEnablement();
}

function changeMaintenanceWindowEnablement(data) {
    if (!data || data.status === "success") {
        var checkbox = Ext.get("editForm:maintenanceEnableCheckbox"),
            checked = Ext.getDom("editForm:maintenanceEnableCheckbox").checked;
        if (checkbox) {
            var div = Ext.get("maintenanceWindowOptions");
            if (div) {
                if (checked) {
...
function changeNativeChangeAttributeSelection() {
    var div = Ext.get("nativeChangesUserDefinedAttributes");
    if ( div ) {
        ...
        if ( selection && selection == "userDefined" ) {
            ...
            div.setVisibilityMode(Ext.Element.DISPLAY).hide();
            var textArea = Ext.get('editForm:nativeChangesUserDefinedAttributesTextArea');
            if ( textArea ) {
                textArea.dom.value = "";
            }
        }
