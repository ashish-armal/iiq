    ...
    statics : {
        getAccountGroupSearchPanel : function(config) {
            ...
            var searchContents = Ext.create('SailPoint.TabContentPanel', {
                ...
                bbar: [{
                    ...
                    handler: function() {
                        Ext.getDom('accountGroupSearchForm:resetBtn').click()
                    }
                }],
                loader: {
        ...
        finishInit : function() {
            ...
            SailPoint.Analyze.registerSubmits({
                ...
                container: Ext.getDom('accountGroupSearchCriteria'),
        ...
        styleSearchPanels : function() {
            buildTooltips(Ext.getDom('accountGroupSearchCriteria'));
            Ext.getCmp('accountGroupSearchContents').doLayout();
        },
        ...
        clearSearchFields : function() {
            ...
            var nameInput = Ext.getDom(formName + ':accountGroupName');
            if (nameInput) {
                nameInput.value = '';
            }
            ...
            if (nativeIdentitySuggest) {
                ...
                Ext.getDom('nativeIdentity').value = '';
            }

            var typeFilterCombo = Ext.getCmp('typeFilterCombo');
            if(typeFilterCombo) {
                typeFilterCombo.clearValue();
                Ext.getDom('accountGroupTypeFilter').value = '';
            }

            Ext.getDom(formName + ':accountGroupTarget').value = '';
            Ext.getDom(formName + ':accountGroupRights').value = '';
            Ext.getDom(formName + ':accountGroupAnnotation').value = '';
        ...
        handleClick : function(gridView, record, HTMLitem, index, e, eOpts){
            ...
            if(colName) {
                Ext.getDom('accountGroupResultForm:searchType').value = 'AccountGroup';
                Ext.getDom('accountGroupResultForm:currentObjectId').value = record.getId();
                Ext.getDom('accountGroupResultForm:editButton').click();
            }
        },

        editAccountGroup : function(item, e) {
            Ext.getDom('accountGroupResultForm:searchType').value = 'AccountGroup';
            Ext.getDom('accountGroupResultForm:currentObjectId').value = item.accountGroupId;
            Ext.getDom('accountGroupResultForm:editButton').click();
        },
            ...
            if (!identityWin) {
                ...
                identityStore.on('metachange', function(changedStore, newMetaData) {
                    ...
                    for (i = 0; i < columnConfig.length; ++i) {
                        ...
                        // use this loop to encode column header
...
Ext.define('SailPoint.Analyze.AccountGroup', {
    statics : {
        initializeAttributes : function(isGrid) {
            ...
            var attributeVal;
            var typeFilterCombo = Ext.getCmp('typeFilterCombo');
            var typeFilterVal = Ext.getDom('accountGroupTypeFilter') ? Ext.getDom('accountGroupTypeFilter').value : '';
            var ownerVal = Ext.getDom('accountGroupOwnerVal').innerHTML;
            var appVal =  Ext.getDom('accountGroupAppVal').innerHTML;
            var ownerSuggest = Ext.getCmp('accountGroupOwnerSuggest');
            var ownerRecords = Ext.getDom('accountGroupOwnerSuggestInit') ? Ext.decode(Ext.getDom('accountGroupOwnerSuggestInit').innerHTML) : { objects: [] };
            var applicationSuggest = Ext.getCmp('accountGroupApplicationSuggestCmp');
            var appRecords = Ext.getDom('accountGroupAppSuggestInit') ? Ext.decode(Ext.getDom('accountGroupAppSuggestInit').innerHTML) : { objects: [] };
            var nativeIdentitySuggest = Ext.getCmp('nativeIdentitySuggestCmp');
            var nativeIdentityVal = Ext.getDom('nativeIdentityVal').innerHTML;
            ...
            if(isGrid) {
                ...
                attributeVal = Ext.getDom('acctGroupAttributeVal').innerHTML;
            ...
            typeFilterCombo = Ext.create('Ext.form.field.ComboBox', {
                ...
                listeners: {
                    change: function(combo, val, oldVal, opts) {
                        ...
                        Ext.getDom('accountGroupTypeFilter').value = newVal;
                        if(this.isGrid) {
                            ...
                            if (show) {
                                ...
                                if (Ext.getDom('editForm:accountGroupTarget').value == '' && combo.target) {
                                    Ext.getDom('editForm:accountGroupTarget').value = combo.target;
                                    combo.target = undefined;
                                }

                                if (Ext.getDom('editForm:accountGroupRights').value == '' && combo.rights) {
                                    Ext.getDom('editForm:accountGroupRights').value = combo.rights;
                                    combo.rights = undefined;
                                }

                                if (Ext.getDom('editForm:accountGroupAnnotation').value == '' && combo.annotation) {
                                    Ext.getDom('editForm:accountGroupAnnotation').value = combo.annotation;
                                    combo.annotation = undefined;
                                }
                            } else {
                                // Clear the inputs but save them in case we need them back later
                                combo.target = Ext.getDom('editForm:accountGroupTarget').value;
                                Ext.getDom('editForm:accountGroupTarget').value = '';
                                combo.rights = Ext.getDom('editForm:accountGroupRights').value;
                                Ext.getDom('editForm:accountGroupRights').value = '';
                                combo.annotation = Ext.getDom('editForm:accountGroupAnnotation').value;
                                Ext.getDom('editForm:accountGroupAnnotation').value = '';
                            }
                        }
                    }
                }
            ...
            Ext.each(extendedFields, function (field) {
                var id = field.id,
                    ...
                    className = 'Identity',
                    suggest = Ext.getCmp(simpleId + 'SuggestCmp'),
                    identityData = Ext.get(simpleId + 'Identity'),
                    suggestVal = field.innerHTML,
                    params = {
                        attrType: Ext.getDom(simpleId + 'Type').innerHTML
                    },
                    prevSib = Ext.fly(field).prev("input"),
                    nextSib = Ext.fly(field).next("div[id*=" + simpleId + "]");
                ...
                var suggestConfig = {
                    ...
                    valueField: 'displayName',
                    width: 200,
                    freeText: false, //bug27956 removed Setting freeText = false to prevent scrollbar collapse in suggest field value browsing
                    className: className,
                ...
                if (identityData) {
                    identityData = identityData.getHTML();
                    if(identityData && identityData !== "") {
                        suggestConfig.initialData = [Ext.JSON.decode(identityData)];
                    }
                }

                // SuggestFactory will return either a DistinctRestSuggest or a ManagedAttributeIdentitySuggest
                // based on the value of extraParams.attrType.
                suggest = SailPoint.SuggestFactory.createSuggest('managedAttribute', simpleId + 'Suggest', null, '', suggestConfig);
