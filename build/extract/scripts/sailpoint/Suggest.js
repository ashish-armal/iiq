...
Ext.define('SailPoint.Suggest', {
    ...
    initComponent : function() {
        ...
        this.on('enable', function() {
            if (this.binding)
                Ext.getDom(this.binding).disabled = false;
        });

        this.on('disable', function() {
            if (this.binding)
                Ext.getDom(this.binding).disabled = true;
        });
...
SailPoint.Suggest.IconSupport = {
    ...
    setIconClsWithLookup : function(suggest) {
        if (suggest.binding && Ext.getDom(suggest.binding)) {
            var id = Ext.getDom(suggest.binding).value;
            if (id) {
...
Ext.define('SailPoint.IdentitySuggest', {
    ...
    // IIQETN-5108 Attaching an event listener to look at the onClick event
    initEvents:function(){
        var me=this;
        me.callParent(arguments);
        me.mon(me.inputEl, 'click', me.checkForMaybeTrigger, me);
    },

    /* IIQETN-5108 Checking in the onClick event if the Identity Suggest Combo Box's
       picker is expanded, and then making sure that its zero index is enough to keep it
       at the same level than the holding window. */
    checkForMaybeTrigger:function(){
        var me=this;
        if(me.getFocusEl()===me.inputEl){
            if(!me.isExpanded){
                me.onTriggerClick()
            } else {
                me.picker.getEl().setStyle('z-index','29002')
            }
        }else{
            me.onTriggerClick()
        }
    ...
    initComponent : function() {
        ...
        if(this.initialData) {
            // If the data is coming in as a raw string decode it
            if (Ext.isString(this.initialData)) {
                this.initialData = Ext.JSON.decode(this.initialData);
            }
        ...
        if (loadBindings) {
            this.store.on('beforeload', function(store, options) {
                ...
                for (loadParam in loadBindings) {
                    ...
                    value = Ext.getDom(loadBindings[loadParam]).value;
        ...
        if(this.initialData && this.initialData.length > 0){
            ...
            this.on('afterrender', function(){
                // If valueField is name use name otherwise use default id
                var initialValue = this.valueField === 'name' ? this.initialData[0].name : this.initialData[0].id;
        ...
        this.getStore().load({
            ...
            callback:function(r, o, s) {
                if(r && r[0] && r[0].get('name') == o.name) {
                    //IIQBUGS-55 if the valueField is the 'name' we need to use that.
                    var value = r[0].getId();
	...
	getSuggestTpl : function(displayField) {
        return '<tpl for=".">' +
                   '<div class="search-item">' +
                       '<div class="sectionHeader">{displayableName:htmlEncode}</div>' +
                       '<div class="font10">#{msgs.name}: {name:htmlEncode}</div>' +
...
Ext.define('SailPoint.BaseSuggest', {
    ...
    initComponent : function() {
        ...
        if (params['suggestType'] === 'application' && typeof (this.initialData) !== 'undefined' && typeof (this.initialData) === 'object') {
            ...
            //check if the value is set while scrolling through the page.  We're assuming false.
            var valueSet = false;
            ...
            if (!valueSet) {
                //The value is likely on another page.  Don't fuss too much, just set the raw value.
                //The component will still behave as it should.
                this.setRawValue(this.initialData);
            }
        }
    }
});
...
Ext.define('SailPoint.ManagedAttributeSuggest', {
    ...
    getSuggestTpl : function(displayField) {
        return '<tpl for="."><div class="x-combo-list-item"><div class="sectionHeader">{displayValue:htmlEncode}</div>'
...
Ext.define('SailPoint.ManagedAttributeNameSuggest', {
    ...
    getSuggestTpl : function(displayField) {
        return '<tpl for="."><div class="x-combo-list-item"><div class="sectionHeader">{attribute:htmlEncode}</div>'
            + '<div class="indentedColumn">{application-name:htmlEncode}</div></div></tpl>';
    },
...
/**
 * @class SailPoint.ManagedAttributeIdentitySuggest
 * @extends SailPoint.Suggest
 *
 * Suggest component for managed attributes of type Identity.
 */
Ext.define('SailPoint.ManagedAttributeIdentitySuggest', {
    extend: 'SailPoint.IdentitySuggest',
    alias: 'widget.managedAttributeIdentitySuggest',
    ...
    initComponent: function() {
        ...
        this.on('beforerender', function() {
            // Update the following to work more like a DistinctRestSuggest
            var url = SailPoint.getRelativeUrl('/rest/suggest/column/' + this.className + "/" + this.column);
            this.getStore().originalDataSourceUrl = url;
            this.getStore().getProxy().url = url;
            this.getStore().getProxy().reader.root = 'objects';
            this.getStore().getProxy().reader.totalProperty = 'count';
        }, this, {single: true});
...
SailPoint.SuggestFactory = {
    ...
    createSuggest : function(suggestType, renderTo, exclusionIds, emptyText, config) {
        ...
        case 'managedAttribute':
            if(config.extraParams.attrType && config.extraParams.attrType === 'sailpoint.object.Identity') {
                suggest = new SailPoint.ManagedAttributeIdentitySuggest(suggestConfig);
            }
            else {
                suggest = new SailPoint.DistinctRestSuggest(suggestConfig);
            }
            break;
        default:
