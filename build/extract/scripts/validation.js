...
var Validator = {
    ...
    //This function verifies that at last 'minSelected' are selected in a given select dom element
    validateSelectedValues: function(element, minSelected, errorMessage) {
        var isValid = false;
        if (element != null && element.options) {
    ...
    displayErrors: function(errorDiv) {
        if (errorDiv) {
            ...
            if (Validator.errorMessages[0] != null) {
                ...
                errorDiv.dom.innerHTML = errorTable;
                errorDiv.dom.style.visibility = 'visible';
    ...
    hideErrors: function(errorDiv) {
        Ext.get(errorDiv).setVisibilityMode(Ext.Element.DISPLAY).hide();
    },
    ...
    validateGreaterThanOrEqual: function(inputFieldId, minValue, errorMessage) {
        var val = new Number(Ext.getDom(inputFieldId).value);
    ...
    // This function verifies that the specified string element is contains the character '='.
    // If the validation fails, the specified errorMessage is added to the error message queue.
    validateDelimitedKeyValuePair : function(element, errorMessage) {
		var properties = element.split(/\r|\n/);
		var isValid;
		for (var i = 0; i < properties.length; i++) {
			if ((properties[i].match(/=/g) || []).length === 1) {
